...
/**
 * Claude Desktop Tool Handler: slide_get_virtual_machines
 * This script is executed by Claude Desktop when the slide_get_virtual_machines tool is called
 */
...
async function getVirtualMachines() {
  try {
    ...
    const result = await slideClient.getVirtualMachines(params);
    
    // Format the result for better readability in Claude's response
    const formattedResult = {
      virtual_machines: result.data.map(vm => ({
        id: vm.virt_id,
        snapshot_id: vm.snapshot_id,
        device_id: vm.device_id,
        state: vm.state,
        memory_mb: vm.memory_in_mb,
