...
/**
 * Claude Desktop Tool Handler: slide_update_device
 * This script is executed by Claude Desktop when the slide_update_device tool is called
 */
...
if (args.hostname) updateData.hostname = args.hostname;
if (args.client_id) updateData.client_id = args.client_id;

// Check if we have data to update
if (Object.keys(updateData).length === 0) {
  console.error('ERROR: At least one of display_name, hostname, or client_id must be provided');
  process.exit(1);
}
...
async function updateDevice() {
  try {
    const result = await slideClient.updateDevice(args.device_id, updateData);
