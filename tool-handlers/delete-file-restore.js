...
/**
 * Claude Desktop Tool Handler: slide_delete_file_restore
 * This script is executed by Claude Desktop when the slide_delete_file_restore tool is called
 */
...
async function deleteFileRestore() {
  try {
    ...
    const result = await slideClient.deleteFileRestore(args.file_restore_id);
    
    // Format the result for better readability in Claude's response
    const formattedResult = {
      success: true,
      message: `File restore ${args.file_restore_id} has been deleted`
    };
