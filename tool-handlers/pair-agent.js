...
/**
 * Claude Desktop Tool Handler: slide_pair_agent
 * This script is executed by Claude Desktop when the slide_pair_agent tool is called
 */
...
if (!args.pair_code) {
  console.error('ERROR: pair_code is required');
  process.exit(1);
}
...
async function pairAgent() {
  try {
    const result = await slideClient.pairAgent(pairData);
