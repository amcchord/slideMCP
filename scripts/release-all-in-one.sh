...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  $0 --dry-run v2.5.0     # Test release process"
            echo "  $0 --no-push            # Build locally without GitHub"
            exit 0
            ;;
        *)
...
sign_and_notarize_macos() {
    ...
    if [[ "$OSTYPE" != "darwin"* ]]; then
        log_warning "Skipping macOS code signing (not on macOS)"
        return
    fi
    
    # Check for Apple credentials
    if [ -z "$APPLE_ID" ] || [ -z "$APP_SPECIFIC_PASSWORD" ]; then
        log_warning "Skipping macOS notarization (credentials not set)"
        log_info "Set APPLE_ID and APP_SPECIFIC_PASSWORD environment variables to enable notarization"
        return
    fi
    ...
    log_success "macOS binaries signed and notarized"
}
